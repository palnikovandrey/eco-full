...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (deleted) {
      ...
      const filtered = parsed.filter((p: any) => now - p.deletedAt < 30 * 24 * 60 * 60 * 1000);
  ...
  const restoreProject = (number: string) => {
  ...
  const exportPDF = () => {
    ...
    projects.forEach((p, i) => {
      ...
      doc.text(\`Адрес: \${p.region}, \${p.district}, \${p.city}, \${p.street}\`, 10, 28 + offset);
    ...
    const paragraphs = projects.map(p =>
      new Paragraph({
        children: [
          ...
          new TextRun(\`\nНазвание: \${p.name}\`),
          new TextRun(\`\nТип: \${p.type}\`),
          new TextRun(\`\nАдрес: \${p.region}, \${p.district}, \${p.city}, \${p.street}\`),
          new TextRun(\`\nЗаказчик: \${p.customer}\`),
        ...
        {view === "list" && (
          <>
            <div className="flex gap-2">
              <button onClick={exportPDF} className="px-4 py-2 bg-red-600 text-white rounded">Экспорт в PDF</button>
              <button onClick={exportWord} className="px-4 py-2 bg-blue-600 text-white rounded">Экспорт в Word</button>
            </div>
          </>
        )}
      </div>
